...
to ensure that the programmers assumptions about the state of the programmers
...
void foo(int* arr){
    ...
    elements, but what if it was smaller? Well let's check then:
\begin{codeblock}
// remember, this just means int* arr,
// no matter if we the size specified
void foo(int arr[3]){ 
