...
// malloc(sizeof(struct entity));
// monster2 = init_entity(7, 7, 100, '#');
// move_player(NORTH); break;
// for (x = -1; x < WIDTH; x++) printf("=");
...
void move_player(enum direction dir)
{
    struct coord new_pos = {player->pos.x + next_coords[dir].x,
                            player->pos.y + next_coords[dir].y};
    if (!out_of_bounds(new_pos)) player->pos = new_pos;
}
...
struct entity* init_entity(int x_pos, int y_pos, int health, char symbol)
{
    struct entity* new_ent = malloc(sizeof(struct entity));
    if (new_ent == NULL) exit(EXIT_FAILURE);
